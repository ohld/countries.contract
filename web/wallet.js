const provider = new Web3.providers.HttpProvider("https://rinkeby.infura.io/sF8QaFr5COSzwukN3V2Y")
...
const wallet = {
    ...
    tokens: [],
    init: function (priv) {
        const key = priv || window.localStorage.getItem('ethereum-private-key')
        if (key) {
            this.account = web3.eth.accounts.wallet.add(key)
        } else {
            this.account = web3.eth.accounts.create()
        }
        window.localStorage.setItem('ethereum-private-key', this.account.privateKey)
        this.getBalance()
        return this.account
    },
